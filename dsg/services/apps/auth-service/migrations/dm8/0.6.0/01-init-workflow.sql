...
from DUAL where not exists (select "f_id" from "t_flow_dag" where "f_id" = 616502025026315947);


INSERT INTO "t_flow_dag_version"
...
}', 1777279536561687626 from DUAL where not exists (select "f_id" from "t_flow_dag_version" where "f_id" = 616982011965384363);


INSERT INTO "t_flow_dag_var"
...
where not exists (select "f_id" from "t_flow_dag_var" where "f_id" = 617284477068093151);

INSERT INTO "t_flow_dag_var"
("f_id", "f_dag_id", "f_var_name", "f_default_value", "f_var_type", "f_description")
select 617284477068158687, 616502025026315947, 'userid', '5f88d1e0-3ebb-11f1-8993-261248b384b3', 'string', '' from DUAL
where not exists (select "f_id" from "t_flow_dag_var" where "f_id" = 617284477068158687);


INSERT INTO "t_flow_dag_step"
...
where not exists (select "f_id" from "t_flow_dag_step" where "f_id" = 616982011965580971);
INSERT INTO "t_flow_dag_step"
("f_id", "f_dag_id", "f_operator", "f_source_id", "f_has_datasource")
select 616982011965646507, 616502025026315947, '@workflow/approval', '', 0 from DUAL
where not exists (select "f_id" from "t_flow_dag_step" where "f_id" = 616982011965646507);
INSERT INTO "t_flow_dag_step"
("f_id", "f_dag_id", "f_operator", "f_source_id", "f_has_datasource")
select 616982011965712043, 616502025026315947, '@internal/tool/py3', '', 0 from DUAL
where not exists (select "f_id" from "t_flow_dag_step" where "f_id" = 616982011965712043);


INSERT INTO "t_flow_dag_accessor"
("f_id", "f_dag_id", "f_accessor_id")
select 617257301468096223, 616502025026315947, '266c6a42-6131-4d62-8f39-853e7093701c' from DUAL
where not exists (select "f_id" from "t_flow_dag_accessor" where "f_id" = 617257301468096223);



INSERT INTO "t_bd_resource_r"
...
where not exists (
  select 1
  from "t_bd_resource_r"
  where "f_resource_id" = '616502025026315947:default'
    and "f_resource_type" = 'data_flow'
    and "f_bd_id" = 'bd_public'
);
