...
import styles from './ActionInfo.module.less';
...
const ActionInfo: FC<ActionInfoProps> = ({ knId, atId, detail: data }) => {
  ...
  return (
    <div className={styles['action-info-root']}>
      {/* Basic Info */}
      <div className={styles['info-card']}>
        <div className={styles['card-title']}>{intl.get('Global.basicInfo')}</div>
        {renderItem(intl.get('Global.id'), data.id)}
        {renderItem(
          intl.get('Global.tag'),
          data.tags?.length > 0 ? (
            <div className="g-flex" style={{ flexWrap: 'wrap', rowGap: 10 }}>
