...
const KnowledgeNetworkOverview = () => {
  ...
  const [metricsTotal, setMetricsTotal] = useState<number>(0);
  ...
  const getMetricsTotal = async (knId: string) => {
    try {
      const res = await metricApi.getMetrics(knId, { offset: 0, limit: 1 });
      setMetricsTotal(res?.total_count || 0);
    } catch (error) {
      console.error('getMetricsTotal error:', error);
    }
  };
    ...
    <div className={styles['main-box']}>
      ...
      <div className={styles['main-layout']}>
        <div className={styles['main-side']}>
          ...
          />
          <LinkItem
            icon="icon-dip-zhibiaomoxing"
            isActive={active === 'metric'}
            title={intl.get('Global.metric')}
            url={`${path}/metric`}
            count={metricsTotal}
          />
          <div className={styles.line}></div>
        ...
        <div className={styles['main-content']}>
          <Switch>
            ...
            <Route exact path={`${path}/metric`} render={() => <Metric detail={detail} isPermission={isPermission} />} />
