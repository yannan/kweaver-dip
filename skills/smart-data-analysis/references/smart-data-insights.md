...
user-invocable: true
description: >-
  数据洞察 skill（三个子场景）：数据解读、维度分析、对比分析。当用户要求「解读数据/趋势/异常/贡献/建议」或「多角度/多维度/分布」或「对比/同比/环比」等时使用（且不要求归因报告终态）。
  面向有分析需求的业务用户：用户可见正文以通俗业务语言为主（结论先说清「说明了什么、对谁有用」），技术核对（表名、子问题编号、原样 SQL 等）集中在「附录 / 核对用」或章节后部，避免主文堆砌术语。
  统一执行路径：先按新版 smart-ask-data 第 7～9 步（候选表检索 → 权限校验 → dataview get）收敛可查询的数据视图与字段；再做子场景识别；在拆分子问题之前还须阅读本轮每一启用子场景在 references/ 下的输出模板（data_interpretation.md、dimensional_analysis.md、contrastive_analysis.md 三选一或多选，仅读启用项）；
...
argument-hint:
  - 默认传入用户自然语言任务（含表/业务主题）；kn_id_ask_data 从 SOUL.md 读取。若仅做离线合成，传入多段 smart-ask-data 最终交付原文（每段含 kn_id、SQL、结果、口径）。可提示优先子场景：数据解读 / 维度分析 / 对比分析。
---

# Smart Data Insights（数据洞察：三子场景）

本 skill 的定位：**在可复核的数据证据上**，按所选子场景输出结构化结论。三个子场景 **共享同一套上游编排**（新版 smart-ask-data 第 7～9 步：候选表检索/判权/拉取字段 → **子场景模板查阅** → 子问题清单 → 每子问题第 10～11 步：生成 SQL/执行查询），差异在于 **子问题设计侧重点** 与 **输出章节模板**（分文件维护于 `references/`，**须在拆子问题前阅读对应模板**）。
...
- **读取时机（与「找表/字段」的关系）**：在 **「子问题拆解」步骤开始之前**，须已完成：**①** 至少一次与用户问题相关的新版 smart-ask-data **第 7～9 步**（`kn-schema-search` 找到候选并完成第 8 步权限校验，且对通过的候选执行过 `dataview get` 拿到字段）；**②** **子场景识别**；**③** 对 **每一个已启用子场景**，打开对应 `references/*.md`，至少阅读 **「子问题拆解指引 / 规则」** 与 **「输出模板」** 两节（标题以各文件内 `##` 为准）。**禁止**在未读模板的情况下编写子问题清单。
...
- `smart-attribution-analysis`：**原因归因**、证据包与可选出图；本 skill **不承担**归因报告终态；若用户既要本 skill 子场景又要「为什么 / 根因 / 驱动因素」，总入口应按 `smart-data-analysis` 路由到归因分支。
...
- **拆子问题前的双前置（MUST）**：默认路径下，在编写「子问题清单」之前，必须 **(A)** 至少完成 **一次** 与用户问题相关的新版 smart-ask-data **第 7～9 步**（候选检索 → 判权 → `dataview get` 获取字段结构）；**(B)** 已按 **已启用子场景** 阅读 `references/data_interpretation.md`、`references/dimensional_analysis.md`、`references/contrastive_analysis.md` 中各自的 **子问题指引 + 输出模板**（仅读启用项，路径见上文「三子场景一览」）。
...
- **子问题数量**：须在 **同一 assistant 回复** 内完成的子问题，建议 **2～8 条**（含）；若无法收敛或超出合理上限，应先澄清口径或终止并说明边界。
...
```text
数据洞察进度：
- [ ] 0. 前置：已由 smart-data-analysis 确认本任务为「数据洞察（三子场景之一或多）」且 kn_id_ask_data / 运行上下文可用（见 smart-ask-data Step 1）
- [ ] 1. 候选表检索（第 7 步）：使用 `kweaver context-loader kn-schema-search "用户问题关键词"` 检索候选表/视图，并按要求输出「候选表列表」（含对象类型/业务名称、dataview_id）。若报 `view_detail` 权限不足或 `DefaultSmallModelEnabled is false` → 失败即停
- [ ] 2. 查询权限校验（第 8 步）：对候选表列表逐一校验 `operations` 是否包含 `data_query`；若全部不含 → 失败即停
- [ ] 3. 获取候选表详情（第 9 步）：对通过权限校验的 `dataview_id` 执行 `kweaver dataview get <dataview_id>`，拿到 `meta_table_name` 与字段信息，作为后续 SQL 生成依据
- [ ] 4. 子场景识别：从三子场景中择一或多；多子场景时见下文「多子场景输出顺序」
- [ ] 5. 查阅子场景输出模板（须在子问题拆解前完成）：对步骤 4 **每一个已启用**子场景，阅读对应 `references/data_interpretation.md` / `references/dimensional_analysis.md` / `references/contrastive_analysis.md` 中的 **子问题拆解指引** 与 **输出模板** 章节；仅读启用项
- [ ] 6. 子问题拆解：写出「子问题清单」（每条可独立生成 SQL 并执行查询回答，且口径互不矛盾）；须能覆盖步骤 5 已读模板中的必选小节，并与步骤 3 的字段信息可落地
- [ ] 7. 逐子问题取数（第 10～11 步）：每个子问题在已获取的候选表详情基础上生成可执行 SQL（含日期合法性校核、默认 `LIMIT 200` 等约束），并仅使用 `kweaver dataview query <dataview_id> --sql <sql_var> --limit 200` 执行查询返回结果（失败即停）
...
- 仅当用户问题**明确**只要“分布/TopN/多角度拆解”时，可只启用「维度分析」；仅当用户问题**明确**只要“对比/同比/环比”时，可只启用「对比分析」。

**说明**：若首轮难以预判子问题能否支撑某模板小节，仍应先按用户意图启用子场景并完成 **步骤 3 模板查阅** 与 **步骤 4 子问题设计**；若取数完成后某子场景章节确不适用，在正文中按该子场景模板写「不适用」并符合其失败/局限小节。
...
**多子场景输出顺序（MUST）**：**数据解读 → 维度分析 → 对比分析**（仅输出已启用的章节，未启用则整章省略或写「本轮不适用」）。若用户 **显式**指定章节顺序（例如「先对比后维度」），以用户指定为准并在「口径与证据来源」中声明。

**与原 v1.1 的对应关系**：仅启用 **维度分析 + 对比分析** 两子场景时，章节顺序与旧版「先第 2 章维度、再第 3 章对比」一致（中间可能多一章数据解读）。
...
- **第 7 步无可用候选 / 失败即停类报错**：若 `kn-schema-search` 无命中，或出现 `Access denied: insufficient permissions for [view_detail]`、`DefaultSmallModelEnabled is false` 等失败即停错误 → 立即终止并按 smart-ask-data 要求提示授权/配置；**禁止**猜表名继续。
- **第 8 步无查询权限**：若所有候选的 `operations` 均不含 `data_query`（或接口失败无法判权且无法补救）→ 立即终止并提示需授权开通数据查询权限；**禁止**绕过判权进入查询。
- **任一子问题第 11 步无有效行**（在 `smart-ask-data` 空结果重试耗尽后）：须在正文中标明该子问题「无数据」，依赖该子问题的子场景章节写「不适用 / 不可判定」，**禁止**用其它查询结果顶替。
- **与归因诉求冲突**：若用户同时要求「原因 / 为什么 / 归因」，总入口应路由到 `smart-attribution-analysis`；本 skill **不**输出归因终态报告。
