...
本 skill 的定位是：**把“找表结果 / 单次取数结果 / 数据洞察最终交付 / 归因分析最终交付”写成一份可交付、可复核的报告**。它**不执行**任何检索、SQL或代码计算；**只消费输入**（即你已经从 `smart-search-tables` / `smart-ask-data` / `smart-data-insights`（含 **归因分析**子场景证据包）拿到的结果。
