...
本 skill 的定位是：**把“找表结果 / 单次取数结果 / 归因分析最终交付”写成一份可交付、可复核的报告**。它**不执行**任何检索、SQL或代码计算；**只消费输入**（即你已经从 `smart-search-tables` / `smart-ask-data` / `smart-attribution-analysis` 拿到的结果）。

你可以把它理解为一个“报告写作层”：

- 输入：来自 `smart-search-tables` / `smart-ask-data` / `smart-attribution-analysis` 的 **原始输出**（含表清单、职责要点、SQL、结果集、子问题证据等）
...
| 报告内顺序 | 步骤内容 |
| --- | --- |
| 1 | 识别报告场景（`table_search` / `sql_query` / `attribution_analysis_report`） |
...
`[smart-reporting] 进度：已完成第 S+6 步（输出最终报告）；流程完成`

示例：接在问数之后（S=12）：

```text
[smart-reporting] 进度：已完成第 13 步（识别报告场景）；下一步：第 14 步（校验输入完整性）
…
[smart-reporting] 进度：已完成第 18 步（输出最终报告）；流程完成
```
...
本 skill **不调用** `json2plot` 等出图工具；若需可视化，使用 Markdown 代码块 **` ```mermaid `** … **` ``` `** 嵌入 Mermaid 源码，且**全部数据点**来自 `smart-search-tables` / `smart-ask-data` / `smart-attribution-analysis`（场景 `attribution_analysis_report` 的输入证据）的输入原文可复核。
...
**可复制填空骨架**：与下列三套框架一一对应的 Markdown 子模版见 [`../assets/README.md`](../assets/README.md)。

下面给出三套“报告框架 + 注意事项”。你把对应场景的输入贴进来，本 skill 就按该模板生成报告。
