# 第10步：复杂查询数据（smart-ask-data）

本文件定义 `smart-ask-data` 第 10 步在“复杂查询”场景下的执行规范。

分支边界声明：本文件仅适用于第 10 步复杂查询分支，不得跨分支改用简单筛选命令替代执行。

## 适用场景

当第 10 步完成分流判断后，若问题不满足“简单条件筛选问题”判定，必须执行本文件流程。

## 执行目标

根据第 9 步获取的候选表详情，生成可执行 SQL，并通过 `kweaver dataview query` 返回可复核结果。
