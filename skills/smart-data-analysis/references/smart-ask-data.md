...
- [ ] 待完成 · 步骤三（寻找候选表并输出候选列表，第 7 步）
- [ ] 待完成 · 步骤四（校验候选表数据查询权限，第 8 步）
- [ ] 待完成 · 步骤五（获取候选表详情，第 9 步）
- [ ] 待完成 · 步骤六（查询类型判定与查询执行，第 10 步）
- [ ] 待完成 · 步骤七（画图需求分支，第 11 步，按需触发）
- [ ] 待完成 · 步骤八（总结结果，第 12 步）
- [ ] 待完成 · 步骤九（输出流程完成态）
...
- 若流程在第 12 步结束，必须在清单中体现“步骤 12（总结结果）已完成”且其余后续项为空，并标注流程完成。
