...
- **问数分支约束（仅 `smart-ask-data`）**：问数命中后必须输出 SQL 与结果数据；不得在问数分支中扩写为主观洞察、归因结论或 MECE 证据链。凡含「解读/趋势/异常/贡献/建议」「多角度/维度/分布」「对比/同比/环比」「归因/根因/为什么/驱动因素/MECE」等诉求，应路由到 `smart-data-insights`（由对应子场景模板承担正文）；若用户只要单条 SQL 结果且不要求上述输出，则留在问数分支。
...
- [ ] 3. 意图路由：问数 / 找数 / 数据洞察（含解读、维度、对比、归因）/ 报告 / 其他；若路由为**问数**，在**进入第 4 步前**对可识别的**公历日期/区间**做合法性校核（见「意图路由」专条）
